...
export default function Pesagem() {
  ...
  // 🚀 LÊ AS CONFIGURAÇÕES SALVAS PELA SUA NOVA TELA
  const getConfiguracoesDesconto = () => {
    const configSalva = JSON.parse(localStorage.getItem('configuracoesDesconto') || '{}');
    return {
      tolImpureza: Number(configSalva.toleranciaImpureza) || 1.0,
      fator1: Number(configSalva.fatorUmidade1) || 1.4,
      fator2: Number(configSalva.fatorUmidade2) || 1.5
    };
  };
  ...
  const config = getConfiguracoesDesconto(); // Aplica os fatores dinâmicos aqui!
  ...
  if (produto === 'Milho') {
    ...
    // Usando as variáveis da tela de configurações
    if (u > 14 && u <= 18) descUmidade = (u - 14) * config.fator1;
    else if (u > 18) descUmidade = (4 * config.fator1) + ((u - 18) * config.fator2);
    
    let descImpureza = Math.max(0, i - config.tolImpureza); 
  ...
  const imprimirTicket = (item: any) => {
    ...
    // 🚀 LÊ AS CONFIGURAÇÕES NA HORA DE IMPRIMIR TAMBÉM
    const cfg = getConfiguracoesDesconto();

    if (item.produto === 'Milho') {
      if (u > 14 && u <= 18) descUmidadePerc = (u - 14) * cfg.fator1;
      else if (u > 18) descUmidadePerc = (4 * cfg.fator1) + ((u - 18) * cfg.fator2);
      descImpurezaPerc = Math.max(0, i - cfg.tolImpureza);
    } else {
      descUmidadePerc = u;
      descImpurezaPerc = i;
    }
